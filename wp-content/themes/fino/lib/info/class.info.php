...
if ( ! class_exists( 'Fino_Info' ) ) {
    ...
    class Fino_Info {
         ...
         * Single instance.
         *
         * @var Fino_Info $instance Instance object.
         */
        ...
        public static function init( $config ) {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Fino_Info ) ) {
                self::$instance = new Fino_Info;
        ...
        public function configure() {
            ...
            $this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s info', 'fino' ), $this->theme_name );
            $this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s info', 'fino' ), $this->theme_name );
            $this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
            $this->page_slug     = $this->theme_slug . '-details';
            $this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
            $this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme details page.', 'fino' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'fino' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'fino_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'fino' ) . '</a></p>';
        }
        ...
        public function render_current_tab_content() {
            ...
            if ( method_exists( $this, $method ) ) {
                ...
                printf( esc_html__( '%s() method does not exist.', 'fino' ), $method );
            }
        }
        ...
        public function useful_plugins() {
            ...
            <div class="fpw-section fpw-section-useful-plugins">
                ...
                <?php if ( true !== $tgmpa_complete ) : ?>
                    <a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'fino' ); ?></a>
                <?php endif; ?>
                ...
                <div class="compare-table table-responsive">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <th class="db-bk-color-one"><?php _e('Product Features', 'fino'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Free', 'fino'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Premium', 'fino'); ?></th>
                        </tr>
                        </thead>
                        <tbody>
